 ...
 */
class ResetManager(private val plugin: BetterTrialChambers) {
    ...
    // (when global.reset-require-confirmation is on). Confirmed via /trial reset confirm.
    ...
    private fun notifyAdminsPending(chamber: Chamber) {
        plugin.scheduler.runTask(Runnable {
            Bukkit.getOnlinePlayers()
                .filter { it.hasPermission("btc.admin.reset") }
                .forEach { p ->
    ...
    suspend fun resetChamber(
        ...
        if (!inProgress.add(chamber.id)) {
            // Visible (not fine) so admins running /trial reset see WHY it failed when the
    ...
    private suspend fun restoreFromSnapshotBlocks(
        chamber: Chamber,
        snapshot: Map<org.bukkit.Location, com.esmpfun.bettertrialchambers.models.BlockSnapshot>,
        initiatingPlayer: Player? = null
    ) {
        ...
        if (initiatingPlayer == null &&
            ...
            try {
                com.esmpfun.bettertrialchambers.utils.FaweResetPlacer(plugin).place(snapshot)
