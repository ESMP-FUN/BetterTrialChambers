 ...
 */
class ChamberDiscoveryManager(private val plugin: BetterTrialChambers) {
    ...
    private data class PendingSeed(
        ...
        val method: com.esmpfun.bettertrialchambers.api.events.ChamberDiscoveredEvent.Method
    ) {
    ...
    fun seed(
        world: World,
        x: Int, y: Int, z: Int,
        method: com.esmpfun.bettertrialchambers.api.events.ChamberDiscoveredEvent.Method =
            com.esmpfun.bettertrialchambers.api.events.ChamberDiscoveredEvent.Method.CHUNK_LOAD
    ) {
    ...
    private fun sweepChunk(world: World, chunk: org.bukkit.Chunk) {
        ...
        for (state in tileEntities) {
            ...
            seed(world, state.x, state.y, state.z,
                com.esmpfun.bettertrialchambers.api.events.ChamberDiscoveredEvent.Method.STARTUP_SWEEP)
    ...
    private suspend fun mergeIntoExisting(
        world: World,
        existing: com.esmpfun.bettertrialchambers.models.Chamber,
        result: BfsResult,
        key: String,
        method: com.esmpfun.bettertrialchambers.api.events.ChamberDiscoveredEvent.Method,
        ...
        if (plugin.config.getBoolean("discovery.notify-ops", true)) {
            ...
            plugin.scheduler.runTask(Runnable {
                plugin.server.onlinePlayers
                    .filter { it.hasPermission("btc.discovery.notify") }
    ...
    private suspend fun registerNew(
        ...
        val discoveryEvent = com.esmpfun.bettertrialchambers.api.events.ChamberDiscoveredEvent(
            world = world,
            suggestedName = name,
            minCorner = corner1,
            maxCorner = corner2,
        ...
        // Lazy-admin safety net: most operators will never run `/trial scan add`, so
