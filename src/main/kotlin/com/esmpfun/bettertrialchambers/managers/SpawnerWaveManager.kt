 ...
 */
class SpawnerWaveManager(private val plugin: BetterTrialChambers) {
     ...
     * custom-provider). Used by [com.esmpfun.bettertrialchambers.listeners.MobInfightingListener]
     ...
     */
    fun clearWavesInChamber(chamber: com.esmpfun.bettertrialchambers.models.Chamber) {
     ...
     * [com.esmpfun.bettertrialchambers.api.events.ChamberClearedEvent]
     ...
     * bounds change (discovery merge), after `/trial scan`, or when a spawner
     ...
     * bounds updates (discovery merges), `/trial scan`, or a trial-spawner block
     ...
     * non-vanilla [com.esmpfun.bettertrialchambers.providers.TrialMobProvider].
     *
        ...
        plugin.scheduler.runAtLocation(wave.location, Runnable {
            ...
            try {
                participants.forEach { uuid ->
                    ...
                    val dropEvent = com.esmpfun.bettertrialchambers.api.events.TrialKeyDropEvent(
                        location = dropLoc,
                        keyType = keyMaterial,
                        ownerUuid = uuid
                    )
                    ...
                    itemEntity.persistentDataContainer.set(
                        com.esmpfun.bettertrialchambers.listeners.SpawnerKeyDropOwnerListener.OWNER_KEY,
                        org.bukkit.persistence.PersistentDataType.STRING,
                        uuid.toString()
                    )
                    itemEntity.persistentDataContainer.set(
                        com.esmpfun.bettertrialchambers.listeners.SpawnerKeyDropOwnerListener.DROPPED_AT_KEY,
    ...
    private fun configureWildSpawnerCooldownAtCompletion(wave: WaveState) {
        ...
        try {
            ...
            if (state is org.bukkit.block.TrialSpawner) {
                // A preset that declares its own `target-cooldown-length` keeps it; one
                // that omits it follows this server-wide wild cooldown. See
                // PresetCooldownPolicy for the full rule.
                if (com.esmpfun.bettertrialchambers.utils.PresetCooldownPolicy.keepsOwnCooldown(plugin, state)) {
                    if (plugin.config.getBoolean("debug.verbose-logging", false)) {
     ...
     */
    fun getWavesInChamber(chamber: com.esmpfun.bettertrialchambers.models.Chamber): List<WaveState> {
