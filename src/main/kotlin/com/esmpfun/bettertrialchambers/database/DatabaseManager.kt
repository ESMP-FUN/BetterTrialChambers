package com.esmpfun.bettertrialchambers.database
 ...
 * [BetterTrialChambers.databaseManager] directly throughout its own codebase,
 ...
 */
open class DatabaseManager(protected val plugin: BetterTrialChambers) {
    ...
    private fun createSQLiteDataSource(): HikariDataSource {
        ...
        val config = HikariConfig().apply {
            ...
            poolName = "BetterTrialChambers-SQLite"
    ...
    private fun createMySQLDataSource(config: FileConfiguration): HikariDataSource {
        ...
        val hikariConfig = HikariConfig().apply {
            ...
            connectionTestQuery = "SELECT 1"
            poolName = "BetterTrialChambers-MySQL"
    ...
    private suspend fun verifyAndHealSchema() = withContext(Dispatchers.IO) {
        ...
        connection.use { conn ->
            ...
            if ("player_uuid" !in statsCols) {
                ...
                plugin.logger.severe("[schema] Back up the table, then rename its id column to 'player_uuid', or drop '$statsTable' so TCP recreates it. See /trial debug schema.")
                return@use
            }
    ...
    /** Snapshot of every TCP table's live columns, for `/trial debug schema`. */
