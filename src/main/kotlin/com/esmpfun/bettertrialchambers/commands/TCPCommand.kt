...
import com.esmpfun.bettertrialchambers.commands.handlers.GenerateCommand
import com.esmpfun.bettertrialchambers.commands.handlers.GiveCommand
import com.esmpfun.bettertrialchambers.commands.handlers.LootCommand
import com.esmpfun.bettertrialchambers.commands.handlers.MobsCommand
import com.esmpfun.bettertrialchambers.utils.MessageUtil
import com.esmpfun.bettertrialchambers.utils.RegionUtil
...
class TCPCommand(private val plugin: BetterTrialChambers) : CommandExecutor {
    ...
    private val dungeonHandler = com.esmpfun.bettertrialchambers.commands.handlers.DungeonCommand(plugin)
    private val containerHandler = com.esmpfun.bettertrialchambers.commands.handlers.ContainerCommand(plugin)
    private val setupHandler = com.esmpfun.bettertrialchambers.commands.handlers.SetupCommand(plugin, plugin.setupController)
    ...
    private fun handleDebug(sender: CommandSender, args: Array<out String>) {
        if (!sender.hasPermission("btc.admin.reload")) {
    ...
    private fun handleScan(sender: CommandSender, args: Array<out String>) {
        if (!sender.hasPermission("btc.admin.scan")) {
            sender.sendMessage(plugin.getMessageComponent("no-permission"))
            return
        }
    ...
    private fun handleSetExit(sender: CommandSender, args: Array<out String>) {
        if (!sender.hasPermission("btc.admin.create")) {
            sender.sendMessage(plugin.getMessageComponent("no-permission"))
            return
        }
    ...
    private fun handleSnapshot(sender: CommandSender, args: Array<out String>) {
        if (!sender.hasPermission("btc.admin.snapshot")) {
            sender.sendMessage(plugin.getMessageComponent("no-permission"))
            return
        }
    ...
    private fun isSnapshotMissing(chamber: com.esmpfun.bettertrialchambers.models.Chamber): Boolean {
     ...
     * clickable `[Create]` that runs `/trial snapshot create <name>`, plus a `[Create all]` header
     * button. Mirrors `/trial list`. Backs the `[list]` link in the join/periodic snapshot reminder.
     */
    private fun handleSnapshotMissing(sender: CommandSender, pageArg: String?) {
        ...
        plugin.launchAsync {
            ...
            sendPageNav(sender, page, maxPage, "/trial snapshot missing")
        }
    }
    ...
    private fun captureSnapshot(sender: CommandSender, chamberName: String) {
        plugin.launchAsync {
            ...
            sender.sendMessage(plugin.getMessageComponent("snapshot-created",
                ...
                "size" to com.esmpfun.bettertrialchambers.utils.CompressionUtil.formatSize(file.length())
            ))
        }
    }

    private fun handleList(sender: CommandSender, args: Array<out String>) {
        if (!sender.hasPermission("btc.admin")) {
            sender.sendMessage(plugin.getMessageComponent("no-permission"))
            return
        }
        ...
        plugin.launchAsync {
            ...
            sendPageNav(sender, page, maxPage, "/trial list")
        }
    }
    ...
    private fun centerDistanceSq(c: com.esmpfun.bettertrialchambers.models.Chamber, loc: Location): Double {
    ...
    private fun handlePause(sender: CommandSender, args: Array<out String>) {
        if (!sender.hasPermission("btc.admin.pause")) {
            sender.sendMessage(plugin.getMessageComponent("no-permission"))
            return
        }
        if (args.size < 2) {
    ...
    private fun handleMenu(sender: CommandSender, args: Array<out String> = emptyArray()) {
        ...
        if (!sender.hasPermission("btc.admin.menu")) {
            sender.sendMessage(plugin.getMessageComponent("no-permission"))
            return
        }
        if (!plugin.isReady) {
    ...
    private fun handleStats(sender: CommandSender, args: Array<out String>) {
        if (!sender.hasPermission("btc.stats")) {
            sender.sendMessage(plugin.getMessageComponent("no-permission"))
            return
        }
        ...
        plugin.launchAsync {
            if (args.size < 2) {
                ...
                if (!sender.hasPermission("btc.admin.stats")) {
                    sender.sendMessage(plugin.getMessageComponent("no-permission"))
                    return@launchAsync
                }
    ...
    private fun handleLeaderboard(sender: CommandSender, args: Array<out String>) {
        if (!sender.hasPermission("btc.leaderboard")) {
            sender.sendMessage(plugin.getMessageComponent("no-permission"))
            return
        }
        if (!plugin.config.getBoolean("statistics.enabled", true)) {
    ...
    private fun handlePaste(sender: CommandSender, args: Array<out String>) {
        if (!sender.hasPermission("btc.admin.generate")) {
            sender.sendMessage(plugin.getMessageComponent("no-permission"))
            return
        }
