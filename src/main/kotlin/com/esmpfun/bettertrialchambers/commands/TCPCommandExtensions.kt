...
fun handleVault(plugin: BetterTrialChambers, sender: CommandSender, args: Array<out String>) {
    if (!sender.hasPermission("btc.admin.vault")) {
...
fun handleKey(plugin: BetterTrialChambers, sender: CommandSender, args: Array<out String>) {
    if (!sender.hasPermission("btc.admin.key")) {
        sender.sendMessage(plugin.getMessageComponent("no-permission"))
        return
    }
