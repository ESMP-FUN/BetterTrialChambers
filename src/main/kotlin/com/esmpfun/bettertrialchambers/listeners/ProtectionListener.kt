...
class ProtectionListener(private val plugin: BetterTrialChambers) : Listener {
    ...
    private fun tunnelBreakVerdict(chamber: com.esmpfun.bettertrialchambers.models.Chamber, block: org.bukkit.block.Block): TunnelVerdict {
    ...
    fun onWildVaultPlace(event: BlockPlaceEvent) {
        ...
        if (event.player.hasPermission("btc.bypass.vaultplace")) return
    ...
    /**
     * A cushion is not hung on a wall, so the handler above never sees one.
     * 26.3 says a cushion has been taken with [EntityBreakEvent] instead, and
     * with [EntityBreakByEntityEvent] when someone did it by hand. Same shared
     * handler list, same reason for reading the subclass here.
     */
    @EventHandler(priority = EventPriority.HIGH, ignoreCancelled = true)
    ...
    private fun deferToWorldGuard(location: org.bukkit.Location, player: Player): Boolean {
        ...
        return com.esmpfun.bettertrialchambers.utils.WorldGuardHook.grantsBuild(player, location)
    }
}
