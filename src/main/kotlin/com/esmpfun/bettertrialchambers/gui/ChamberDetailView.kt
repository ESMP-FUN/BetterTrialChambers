...
class ChamberDetailView(
    ...
    private fun handleExpandClick(player: Player) {
        if (!player.hasPermission("btc.admin.scan")) {
     ...
     * verbatim via [BetterTrialChambers.getGuiLore].
     */
                ...
                plugin.launchAsync {
                    try {
                        val success = plugin.resetManager.resetChamber(
                            chamber, player,
                            com.esmpfun.bettertrialchambers.api.events.ChamberResetEvent.Reason.FORCED
                        )
                        plugin.scheduler.runAtEntity(player, Runnable {
                            if (success) {
    ...
    private fun handlePauseToggleClick(player: Player) {
        if (!player.hasPermission("btc.admin.pause")) {
            player.sendMessage(plugin.getMessageComponent("no-permission"))
            return
        }
